...
# Packages
RUN apt install -y aircrack-ng amap apktool awscli beef binwalk bloodhound bloodyad burpsuite cewl checksec chromium crackmapexec crowbar crunch curl dbus-x11 default-mysql-client \
    dex2jar dirb dirbuster dnsenum dnsrecon dnsutils dos2unix enum4linux-ng evil-winrm-py exiftool exploitdb fierce ffuf flameshot foremost ftp gcc gdb ghidra git gobuster hashcat \
    ...
    netcat-traditional netexec nfs-common nikto nmap ntpsec-ntpdate ollydbg onesixtyone patator php powercat powershell powersploit proxychains4 python2 python2-dev python3 python3-dev \
...
# Binaries
RUN mkdir -p /opt/bin
RUN wget -q -O /opt/bin/chisel_linux_amd64.gz "$(curl -s https://api.github.com/repos/jpillora/chisel/releases/latest | grep -Eo 'https.*linux_amd64.gz')" && gzip -d /opt/bin/chisel_linux_amd64.gz && chmod +x /opt/bin/chisel_linux_amd64
RUN wget -q -O /opt/bin/chisel_windows_amd64.zip "$(curl -s https://api.github.com/repos/jpillora/chisel/releases/latest | grep -Eo 'https.*windows_amd64.zip')" && unzip /opt/bin/chisel_windows_amd64.zip
...
RUN pip install --no-cache-dir oletools[full] paramiko pexpect playwright pwntools requests websockets

# Zsh
COPY .zsh_history /home/huhu/.zsh_history
...
# Permissions
RUN chown -R huhu:huhu /home/huhu /opt
